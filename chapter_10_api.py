...
async def all_products(request: Request) -> Response:
    async with aiohttp.ClientSession() as session:
        ...
        # Если запрос к сервису товаров завершился по тайм-ауту, вернуть ошибку, потому что
        ...
        # и использовать их для получения информации о наличии
        else:
...
# Получив данные о товарах, отправить запросы о наличии
async def get_products_with_inventory(session: ClientSession, product_response) -> List[Dict]:
    ...
    for done_task in inventory_done:
        if done_task.exception() is None:
            ...
            product_results.append(create_product_record(
                product_id, inventory['inventory']))
        else:
            product_id = inventory_tasks_to_product_id[done_task]
...
# Вспомогательная функция для получения числа элементов в массиве JSON,
